...
func SignalHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Si pas de token dans le header, essayer dans l'URL
		if token == "" {
			token = c.Query("token")
		}
		...
		log.Printf("Looking for room %s in memory: %v", roomID, ok)
		if !ok {
			if mongoClient != nil {
				...
				log.Printf("Looking for room %s in database: %v", roomID, err)
				if err == nil {
					room = &dbRoom
					room.Participants = make(map[string]*models.Participant)
					rooms[roomID] = room
					log.Printf("Room %s loaded from database", roomID)
				} else {
					roomsMu.Unlock()
				...
				roomsMu.Unlock()
				log.Printf("Room %s not found in memory and no MongoDB client", roomID)
				ws.WriteJSON(WSMessage{Type: "error", Data: "Room not found"})
				return
			}
		}
		...
		log.Printf("Added participant %s (%s) to room %s. Total participants: %d", username, userID, roomID, len(room.Participants))
		roomsMu.Unlock()

		broadcast(roomID, WSMessage{Type: "join", Data: map[string]string{"user": username}})
		log.Printf("Broadcasting join message for %s in room %s", username, roomID)
		sendParticipantList(room)
		log.Printf("Sending participant list for room %s", roomID)
