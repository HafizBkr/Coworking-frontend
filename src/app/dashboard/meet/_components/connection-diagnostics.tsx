...
export function ConnectionDiagnostics() {
  ...
  const runDiagnostics = async () => {
    ...
    // 3. Vérifier le roomId
    results.push({
      name: 'ID de salle',
      status: roomId ? 'success' : 'error',
      message: roomId ? 'ID de salle présent' : 'ID de salle manquant',
      details: roomId || 'Aucun roomId dans l\'URL'
    });
    ...
    try {
      ...
      results.push({
        ...
        message: response.ok ? 'Connexion internet OK' : 'Problème de connectivité',
    ...
    try {
      ...
      results.push({
        ...
        details: isHealthy ? 'Le serveur répond correctement' : 'Le serveur ne répond pas'
      });
    } catch (error) {
      results.push({
        name: 'Serveur de visioconférence',
    ...
    // 6. Tester la création de WebSocket (sans se connecter)
          ...
          <div className="mt-4 p-3 bg-muted rounded-lg">
            ...
            <div className="flex items-center space-x-4 text-sm">
              <span className="flex items-center space-x-1">
                ...
                <span>{diagnostics.filter(d => d.status === 'success').length} OK</span>
              </span>
              <span className="flex items-center space-x-1">
                <XCircle className="h-4 w-4 text-red-500" />
                <span>{diagnostics.filter(d => d.status === 'error').length} Erreurs</span>
              </span>
              <span className="flex items-center space-x-1">
                <AlertCircle className="h-4 w-4 text-yellow-500" />
                <span>{diagnostics.filter(d => d.status === 'warning').length} Avertissements</span>
              </span>
            </div>
          </div>
        )}
