...
import { VisioService, Participant, WSMessage } from '../_services/visio.service';
...
export function useVideoConference() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!targetRoomId || targetRoomId === 'test' || targetRoomId === 'new') {
        console.log('[useVideoConference] Creating new room for workspace:', currentWorkspace._id);
        ...
        try {
          ...
          console.log('[useVideoConference] Room created successfully:', targetRoomId);
      ...
      console.log('[useVideoConference] Starting connection to room:', targetRoomId);
      
      // Créer la connexion WebSocket selon la doc
      const ws = await VisioService.createWebSocketConnection(targetRoomId, session.token);
      wsRef.current = ws;
      ...
      ws.onerror = (error) => {
        ...
        console.error('[useVideoConference] WebSocket readyState:', ws.readyState);
  ...
  }, [roomId, actualRoomId, session?.token, currentWorkspace?._id, isConnecting, isConnected]);
  ...
  const handleWebSocketMessage = useCallback((message: WSMessage) => {
    switch (message.type) {
      ...
      case 'screen_share_start':
        ...
        console.log('[useVideoConference] User started screen share:', message.data.user);
        break;
  ...
  const toggleMute = useCallback(() => {
    const isMuted = participants.find(p => p.userID === session?.user?.id)?.audioMuted;
    sendMessage(isMuted ? 'unmute' : 'mute');
  }, [participants, session?.user?.id, sendMessage]);

  const toggleVideo = useCallback(() => {
    const isVideoOff = participants.find(p => p.userID === session?.user?.id)?.videoOff;
    sendMessage(isVideoOff ? 'video_on' : 'video_off');
  }, [participants, session?.user?.id, sendMessage]);

  const toggleScreenShare = useCallback(() => {
    const isScreenSharing = participants.find(p => p.userID === session?.user?.id)?.screenSharing;
